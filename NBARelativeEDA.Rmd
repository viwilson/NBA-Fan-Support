...
NBATeams %>%
  filter(Team == "GoldenStateWarriors") %>%
  select(Playoffs, pred, Popularity, W.L., MonthNum) %>%
  group_by(Playoffs) %>%
  summarize(Bandwagon = mean(Popularity, na.rm = TRUE) - mean(pred, na.rm = TRUE))

NBATeams %>%
  filter(Team == "NewYorkKnicks") %>%
  select(pred, Popularity, W.L.) %>%
  group_by(W.L.) %>%
  summarize(Bandwagon = mean(Popularity, na.rm = TRUE) - mean(pred, na.rm = TRUE)) %>%
...
```{r}
NBATeams %>%
  select(Playoffs, pred, Popularity, W.L., MonthNum, Team) %>%
...
NBATeams %>%
  select(Playoffs, pred, Popularity, W.L., MonthNum, Team, Season) %>%
...
```{r}
#Computing Bandwagon Score for years in which team did not make client
NBATeams %>% filter(Playoffs == "Did Not Make Playoffs") %>%
  group_by(Team) %>% 
  summarize(Bandwagon = mean(Popularity, na.rm = TRUE) - mean(pred, na.rm = TRUE))
```
...
```{r}
bw_Overall<- bw_NoPlayoffs %>% select(Team)
...
bw_Overall %>% arrange(-Bandwagon)
```
After computing this bandwagon metric, we see that the team that seems to be
most prone to the "Bandwagon effect" is the Golden State Warriors by far, as
there Bandwagon score is much larger than the next closest team (New York Knicks). This indicates that, on average, the Golden State Warriors are much more popular than predicted in years in which they make the playoffs and much
less popular than predicted in years in which they do not make the playoffs.
This is evidence of the bandwagon effect, because it shows that when the Warriors are performing well, they have much more interest than usual, and when
they are not performing well, this interest drastically decreases. 
