...
```{r}
#This model gives an NA estimate for Size variable
...
```{r}
bw_Cats <- bw_Cats %>% left_join(bw_Favorites)
...
bw_Cats$BWScore <- bw_Cats$FavoritesDiff + bw_Cats$ContendersDiff - bw_Cats$DarkHorse
...
gsw.lm <- lm(Popularity~Season, data= gsw.query)
summary(gsw.lm)
```
