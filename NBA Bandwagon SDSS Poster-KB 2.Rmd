...
author: "Victor Wilson and Kelly Bodwin"
...
NBAData$Date <-mdy(NBAData$Date)

NBAData <- NBAData %>% filter(Season < 2017)
...
```{r, echo=FALSE}
NBAData %>%
  filter(Team == "BostonCeltics" & year(Date) < 2019) %>%
  ...
  ggtitle("Boston Celtics Actual vs. Predicted Popularity") + theme(plot.title = element_text(color="black", size=16, face="bold")) + scale_colour_manual(name="Legend", values = c("Actual" = "black", "Predicted" = "red"))
```
...
plt <- ggplot(NBAData, aes(x = log(Popularity+1), y = Playoffs, fill = Succ_Cat)) + geom_density_ridges(na.rm = TRUE, quantile_lines = TRUE, quantiles = 2, alpha = 0.7) +  theme(axis.text.x=element_text(angle=45, hjust=1)) + 
  guides(fill = FALSE) +
  ggtitle("Popularity Distributions by Season Results")

suppressWarnings(print(plt))

```

Our choice of three categories is justified by an ANOVA test (F = 217, p < 0.001) and subsequent Tukey Honest Significant difference tests (p > 0.05 for pairwise comparisons of “Lost 1st Round - Did Not Make Playoffs”, “Lost Conf. Semis - Lost Conf. Finals”, and “Won Finals - Lost Finals”; p < 0.001 otherwise)
